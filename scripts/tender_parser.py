...
# --------------------------------------------------------------------------
# 1. Профили тематики: коды и ключевые слова с весами (--profile)
# --------------------------------------------------------------------------

PROFILES = {
    # Материалы для полиуретан-цементных промышленных полов (основной профиль
    # Бетоникум). Два типа релевантных тендеров:
    #   а) ПОСТАВКА материалов (наливные полы, полимерные составы) — участвуем сами;
    #   б) РАБОТЫ по устройству полов — заказчик работ покупает материалы,
    #      подрядчик-победитель — наш потенциальный клиент.
    "pufloor": {
        "title": "полиуретан-цементные промышленные полы",
        "okpd2": {
            "20.30.12": 80,  # материалы лакокрасочные и аналогичные для покрытий
            "20.30": 60,
            "20.16.56": 60,  # полиуретаны в первичных формах
            "20.16": 40,
            "23.64": 60,     # смеси и растворы строительные (наливные полы, сухие смеси)
            "20.52": 30,     # клеи
            "43.33": 70,     # работы по устройству покрытий полов
            "43.39": 30,     # прочие отделочные работы
        },
        "keywords": {
            "полиуретан-цемент": 100, "полиуретанцемент": 100,
            "наливн": 80, "полимерн пол": 80, "полимерное покрытие пол": 80,
            "полимерн покрыти": 60, "промышленн пол": 70,
            "эпоксидн": 40, "полиуретанов": 50,
            "элакор": 50, "ucrete": 60, "sikafloor": 50,  # бренды-маркеры сегмента
            "обеспыливани": 30, "топпинг": 30, "упрочнени пол": 30,
            "пищев": 25, "паркинг": 20, "автостоянк": 20, "цех": 15, "склад": 10,
            "поставка материал": 25,  # прямой канал для производителя
        },
        "negative": {
            "линолеум": -40, "ламинат": -40, "ковров": -40, "плитка пвх": -30,
            "кварцвинил": -30, "паркет": -40, "демонтаж": -20,
            "спортивн покрыти": -25, "резинов покрыти": -25,
        },
    },
    # Товарный бетон / ЖБИ (исторический профиль, оставлен для справки)
    "beton": {
        "title": "товарный бетон / ЖБИ",
        "okpd2": {
            "23.63": 100, "23.61": 90, "23.64": 60, "23.69": 40,
            "23.51": 30, "08.12": 20, "23.62": 20,
        },
        "keywords": {
...
LAW_BONUS = {"44": 5, "223": 5, "коммерч": 8}  # коммерческие чуть ценнее (меньше конкуренции)

# Активный профиль (устанавливается в main() из --profile)
OKPD2_WEIGHTS = PROFILES["pufloor"]["okpd2"]
KEYWORD_WEIGHTS = PROFILES["pufloor"]["keywords"]
NEGATIVE_KEYWORDS = PROFILES["pufloor"]["negative"]
PROFILE_TITLE = PROFILES["pufloor"]["title"]


def apply_profile(name: str):
    global OKPD2_WEIGHTS, KEYWORD_WEIGHTS, NEGATIVE_KEYWORDS, PROFILE_TITLE
    p = PROFILES[name]
    OKPD2_WEIGHTS, KEYWORD_WEIGHTS = p["okpd2"], p["keywords"]
    NEGATIVE_KEYWORDS, PROFILE_TITLE = p["negative"], p["title"]
...
<body>
  <h1>Тендеры по тематике «{escape(PROFILE_TITLE)}» — топ по релевантности</h1>
  <div class="sub">betonicum.ru · сформировано: {escape(generated_at)} · позиций: {len(tenders)}</div>
  <div class="legend">
    <b>Релевантность (score)</b> считается автоматически по активному профилю: коды ОКПД2,
    ключевые слова в названии закупки, объём, тип закупки; анти-слова (линолеум, ковровые,
    спортивные покрытия и т.п.) снижают score, отсекая нецелевые лоты.
    Полная логика и веса — в <i>scripts/tender_parser.py</i>.
  </div>
  <table>
    <tr>
...
def build_pdf(tenders, generated_at, pdf_path):
    ...
    story = [
        Paragraph(f"Тендеры по тематике «{PROFILE_TITLE}» — топ по релевантности", h1),
        Paragraph(f"betonicum.ru · сформировано: {escape(generated_at)} · позиций: {len(tenders)}", sub),
        Spacer(1, 4 * mm),
        Paragraph("Релевантность (score) считается автоматически по активному профилю: коды ОКПД2, "
                  "ключевые слова в названии закупки, объём, тип закупки. Анти-слова (линолеум, ковровые, "
                  "спортивные покрытия и т.п.) снижают score. Логика и веса — в scripts/tender_parser.py.",
                  sub),
        Spacer(1, 4 * mm),
    ]
...
def main():
    ...
    ap.add_argument("--profile", choices=sorted(PROFILES), default="pufloor",
